...
<template>
    <div v-if="!isSlugEmpty" class="flex gap-10">
        ...
        <div class="w-1/5 sticky top-0 self-start overflow-auto flex flex-col justify-between h-screen" :style="{ maxHeight: `calc(100vh - ${headerHeight}px - 2.5rem)`, top: `calc(${headerHeight}px + 2.5rem)` }">
            <div>
                Contributors
            </div>

            <nav class="text-sm text-gray-500">
                <p class="uppercase text-gray-500 font-semibold">On this page</p>
                <ul>
                    <li v-for="item in toc" :key="item.id" class="mt-2 mb-4">
                        <a @click.prevent="handleTocItemClick(item.id)" href="#" class="hover:text-black">{{ item.text }}</a>
                        <ul>
                            <li v-for="child in item.children" :key="child.id" class="ml-3 my-2">
                                <a @click.prevent="handleTocItemClick(child.id)" href="#" class="hover:text-black">{{ child.text }}</a>
                            </li>
                        </ul>
                    </li>
                </ul>
