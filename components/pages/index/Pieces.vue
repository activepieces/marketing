...
<script setup>
const { data: piecesData, status: resultsStatus } = await useFetch(
  "https://cloud.activepieces.com/api/v1/pieces?sortBy=UPDATED&orderBy=DESC"
);

let allPieces = piecesData.value.filter(
  (piece) => piece.categories != null && piece.categories.length > 0
);
      ...
      <div class="gap-8 items-center xl:gap-16 flex flex-col md:flex-row">
        <div class="basis-1/4 text-center px-4 md:text-left">
          ...
          <p class="font-light text-gray-500 dark:text-gray-400 sm:text-xl">
            Our pieces library grows every day through our team and community
            contributions
          </p>
          <NuxtLink
            to="/pieces"
            class="inline-flex items-center mt-6 justify-center px-5 py-3 text-base font-medium text-center text-white rounded-lg bg-primary hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:focus:ring-primary-900"
          >
            Browse our {{ piecesData.length }} pieces
            <svg
        ...
        <div
          class="group w-full md:basis-3/4 flex flex-col space-y space-y-8 overflow-hidden [mask-image:linear-gradient(90deg,#0000,#fff_5%,#fff_95%,#0000)]"
        >
          ...
          <div class="marquee flex overflow-hidden select-none pt-2 pb-1">
            <div
              class="marquee__group shrink-0 flex items-center justify-center min-w-ful group-hover:[animation-play-state:paused]"
            >
              <NuxtLink
                :to="`/pieces/${piece.internalName}`"
