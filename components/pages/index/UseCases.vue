        ...
        <div class="flex flex-col space-y space-y">
            <ul class="grid grid-cols-3 lg:grid-cols-5 justify-stretch w-full gap-4 text-sm font-medium text-gray-500 dark:text-gray-400 md:me-4 mb-4 md:mb-0" id="default-tab" data-tabs-toggle="#default-tab-content" role="tablist">
                <li v-for="(category, index) in [
                    ...
                    <button class="inline-flex text-center items-center justify-center text-md py-4 lg:text-lg lg:py-5 rounded-lg bg-white text-gray-600 hover:text-black aria-selected:text-white aria-selected:hover:text-white aria-selected:bg-primary-600 w-full dark:bg-gray-800 dark:hover:bg-gray-700 dark:hover:text-white transition duration-100"
                      :id="category.id +'-tab'" :data-tabs-target="'#'+ category.id" type="button" role="tab" :aria-controls="category.id" :aria-selected="index == 0 ? 'true' : 'false'">
                        {{  category.name  }}
                    </button>
                </li>
            </ul>
            <div id="default-tab-content" class="text-medium text-gray-500 dark:text-gray-400 dark:bg-gray-800 rounded-lg w-full">
                <div v-for="(category, index) in [
                 ...
                 ]" :id="category.id" role="tabpanel" :aria-labelledby="category.id +'-tab'" class="hidden">
                        ...
                        <div v-for="template in marketingTemplates.data.filter((template) => template.attributes.category == category.id)" class="bg-white rounded-lg border border-gray-200 shadow-sm dark:bg-gray-800 dark:border-gray-700">
                            <a :href="`https://cloud.activepieces.com/import-flow-uri-encoded?flow=${encodeURIComponent(template.attributes.template)}`" class="h-full group flex flex-col justify-between px-6 py-6">
                                ...
                                <p class="mt-8 font-light text-gray-500 dark:text-gray-400">
                                    <a class="inline-flex items-center justify-center px-4 py-2 text-base font-medium text-center text-primary rounded-lg bg-primary-100 group-hover:bg-primary group-hover:text-white focus:ring-4 focus:ring-primary-300 dark:focus:ring-primary-900">
                                        Try template
