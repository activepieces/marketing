...
<section class="bg-white dark:bg-gray-900 my-16">
    <div class="mx-auto max-w-screen-xl sm:py-6 lg:px-6">
        <div class="max-w-screen-md mb-2 lg:mb-16">
            <h2 class="mb-4 text-4xl tracking-tight font-bold text-gray-900 dark:text-white">The most powerful tool in your stack</h2>
        </div>

        <div class="gap-8 items-center xl:gap-16 flex" id="features-tab" data-tabs-toggle="#features-tab-content" role="tablist">
            <div class="mt-4 md:mt-0 basis-2/5 flex space-y space-y-10 flex-col self-stretch" roles="presentation">
                <div v-for="(group, gindex) in featureGroups">
                    <h3 class="text-lg font-bold mb-3.5">{{ group.name }}</h3>
                    <div class="flex flex-row flex-wrap">
                        <button v-for="(feature, findex) in group.features" type="button" role="tab" :data-tabs-target="`#f-${gindex}-${findex}`" :aria-controls="`f-${gindex}-${findex}`" aria-selected="false" class="group relative outline-none focus:ring-0 flex whitespace-nowrap items-center py-2.5 px-5 me-4 mb-4 text-lg font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-300 hover:border-gray-300 hover:text-primary focus:z-10 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 aria-selected:bg-primary-50 aria-selected:text-primary">
            ...
            <template v-for="(group, gindex) in featureGroups">
                <div v-for="(feature, findex) in group.features" :id="`f-${gindex}-${findex}`" role="tabpanel" :aria-labelledby="`f-${gindex}-${findex}-tab`" class="hidden basis-3/5 py-14 pl-14 rounded-l-[3rem]" :class="feature.color">
                <h2 class="pr-14 text-4xl tracking-tight font-medium text-gray-900 dark:text-white mb-6">{{ feature.longName  }}</h2>
