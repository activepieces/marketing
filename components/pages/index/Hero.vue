            ...
            <div class="lg:col-span-6 text-center sm:mb-6 lg:text-left lg:mb-0">
                <div class="max-w-screen-sm mx-auto">
                    <a :href="homepageAnnouncement.data.attributes.url" v-if="homepageAnnouncement.data.attributes.content" class="inline-flex max-w-full items-center justify-between px-1 py-1 pr-4 mb-6 text-sm text-gray-700 bg-gray-100 rounded-full dark:bg-gray-800 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700" role="alert">
                        <span class="px-3 py-1 mr-3 text-xs text-white rounded-full bg-primary-600 whitespace-nowrap">{{ (new Date(homepageAnnouncement.data.attributes.updatedAt).toDateString() === new Date().toDateString()) ? "Today" : new Date(homepageAnnouncement.data.attributes.updatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }).replace(/\./g, '') }}</span> <span class="text-sm font-medium overflow-hidden whitespace-nowrap text-ellipsis max-w-full">{{ homepageAnnouncement.data.attributes.content }}</span>
                    ...
                    <p class="max-w-xl mx-auto mb-6 font-light text-gray-900 lg:mx-0 xl:mb-8 md:text-lg xl:text-2xl dark:text-gray-400">Securely deploy the easiest automation tool for your marketing, sales, operations, HR, finance and IT teams</p>
