...
const getPieceIcon = (pieceName) => {
  ...
  const piece = piecesData.value.find(
    (p) =>
      p.name === `@activepieces/piece-${pieceName}` || p.name === pieceName,
  );
  return (
    piece?.logoUrl || `https://cdn.activepieces.com/pieces/${pieceName}.png`
  );
};
      ...
      if (data.value) {
        ...
        if (logoUrl && process.client) {
          try {
            ...
            console.log(
              `Color extraction failed for ${domain}, using API color`,
            );
          }
        }
    ...
    if (data.value.logo && process.client) {
      try {
        ...
        if (extractedColor && extractedColor !== "#3b82f6") {
          ...
          data.value.primaryColor !== "#3b82f6"
        ) {
...
const exponentialChartPath = computed(() => {
  ...
  for (let x = 0; x <= maxX; x += 3) {
    ...
    const exponentialFactor =
      (Math.exp(x / scale) - 1) / (Math.exp(maxX / scale) - 1);
...
<template>
  <section class="dark:bg-gray-900 overflow-x-hidden flex-1 flex flex-col">
    ...
    >
      <!-- Low-quality blurred placeholder - shows instantly -->
      <div
        class="absolute inset-0"
        style="
          background-image: url('data:image/jpeg;base64,');
    ...
    <div class="relative z-20 w-full flex-1 flex flex-col">
      ...
      <div class="main-bg-section relative w-full max-w-full overflow-hidden flex flex-col">
        ...
        <div
          class="hidden fixed bottom-4 right-4 z-50 bg-black/80 backdrop-blur-sm rounded-lg p-3 shadow-lg"
        >
        ...
        <div
          :class="[
            'relative z-20 mx-auto w-full max-w-[1230px] px-4',
            'hero-fade-in',
            isVisible ? 'hero-visible' : '',
          ]"
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div
                    ...
                    style="
                      filter: drop-shadow(0 0 40px rgba(0, 0, 0, 0.3))
                        drop-shadow(0 0 80px rgba(0, 0, 0, 0.2));
                      z-index: 99999;
                      isolation: isolate;
                    "
                    @transitionend="
                  ...
                  >
                    <div
                      class="bg-white/95 backdrop-blur-sm rounded-xl shadow-lg border border-gray-200 p-4 min-w-[320px] relative"
                      style="pointer-events: auto !important"
                    >
                      <p class="text-sm text-gray-700 mb-3 text-center">
                        Use your theme, enter your email
                      </p>
                      <form
                        @submit.prevent="fetchCustomBranding"
                        class="flex gap-2"
                      >
                        <input
                          ref="emailInputRef"
                          ...
                          :style="{
                            '--tw-ring-color':
                              currentOrgData.colors.primary + '40',
                            '--tw-border-opacity': '1',
                          }"
                          @focus="
                            (e) => {
                              e.target.style.borderColor =
                                currentOrgData.colors.primary;
                            }
                          "
                          @blur="
                            (e) => {
                  ...
                  <!-- Left Sidebar -->
                  <div
                    class="w-56 bg-white border-r border-gray-200 flex flex-col select-none"
                  >
                    <!-- Logo Section -->
                    <div
                      class="p-6 border-b border-gray-200 relative pointer-events-auto"
                    >
                      <div class="flex items-center gap-2">
                        <img
                          v-if="currentOrgData.logo"
                          :src="currentOrgData.logo"
                          :alt="currentOrgData.name"
                          class="h-8 w-auto transition-opacity duration-500"
                        />
                        ...
                        >
                          <svg
                            class="w-3 h-3 text-gray-600 pointer-events-none"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
                          ...
                          <span
                            class="absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover/reset:opacity-100 pointer-events-none transition-opacity duration-200 whitespace-nowrap z-[201]"
                          >
                            Reset theme
                            <div
                              class="absolute top-full left-1/2 -translate-x-1/2 w-2 h-2 bg-gray-800 rotate-45 -mt-1"
                            ></div>
                          </span>
                        </button>
                      </div>
                    ...
                    <div class="flex-1 p-4">
                      <div
                        class="px-3 py-2 text-sm font-semibold text-gray-900 rounded-md"
                        :style="{
                          backgroundColor: currentOrgData.colors.primaryLighter,
                        }"
                      >
                        Explore
                      </div>
                      <div
                        class="px-3 py-2 text-sm font-medium text-gray-700 rounded-md mt-1"
                      >
                        Impact
                      </div>
                      <div
                        class="px-3 py-2 text-sm font-medium text-gray-700 rounded-md"
                      >
                        Leaderboard
                      </div>
                      ...
                      <div class="mt-6">
                        ...
                        <div
                          class="px-3 py-2 text-sm text-gray-700 rounded-md mt-1"
                        >
                          Personal Project
                        </div>
                  ...
                  >
                    ...
                    <div class="grid grid-cols-2 gap-4">
                      ...
                      <div
                        class="relative rounded-xl p-4 hover:shadow-lg transition-all duration-300 cursor-pointer group border"
                        style="
                          background: linear-gradient(
                            135deg,
                      ...
                      >
                        <div class="flex items-center justify-between mb-2">
                          ...
                          <button
                            class="px-2 py-1 text-[10px] font-semibold rounded-md text-white transition-all duration-300 group-hover:scale-105"
                            :style="{
                              backgroundColor: currentOrgData.colors.primary,
                            }"
                          >
                            Use
                          </button>
                        ...
                        <div class="flex items-center justify-between">
                          ...
                          <div class="flex items-center gap-2">
                            <img
                              :src="
                                getPieceIcon('zendesk') ||
                                'https://cdn.activepieces.com/pieces/zendesk.png'
                              "
                              alt="Zendesk"
                      ...
                      >
                        ...
                        <div class="flex items-center justify-between">
                          ...
                          <div class="flex items-center gap-2">
                            ...
                            <img
                              :src="
                                getPieceIcon('service-now') ||
                                'https://cdn.activepieces.com/pieces/service-now.png'
                              "
                              alt="ServiceNow"
                              class="w-5 h-5 rounded"
                            />
                      ...
                      <div
                        class="relative rounded-xl p-4 border"
                        style="
                          background: linear-gradient(
                            45deg,
                            rgba(20, 184, 166, 0.08) 0%,
                            rgba(153, 246, 228, 0.08) 100%
                          );
                ...
                >
                  <!-- Chart Popup Card -->
                  <div
                    class="absolute top-20 right-0 w-96 bg-white rounded-xl shadow-2xl pointer-events-none select-none popup-card"
                    style="
                      box-shadow:
                  ...
                  >
                    <div class="p-5">
                      <!-- AI Adoption Section -->
                      <div class="mb-5">
                        <div class="text-xs font-semibold text-gray-900 mb-3">
                          AI Adoption
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                          <!-- Personal Adoption -->
                          ...
                          >
                            <defs>
                              ...
                              >
                                <stop
                                  offset="0%"
                                  :style="`stop-color:${currentOrgData.colors.chartColor};stop-opacity:0.4`"
                                />
                                <stop
                                  offset="100%"
                                  :style="`stop-color:${currentOrgData.colors.chartColor};stop-opacity:0`"
                                />
                              </linearGradient>
                            </defs>
                            <!-- Y-axis labels -->
                  ...
                  <div
                    class="absolute top-64 left-0 w-[300px] bg-white rounded-xl shadow-2xl pointer-events-none select-none popup-card"
                    style="
                      box-shadow:
                        0 25px 80px -12px rgba(0, 0, 0, 0.3),
                        0 0 0 1px rgba(0, 0, 0, 0.05);
                      ...
                      <div class="space-y-4">
                            ...
                            >
                              <span
                                ...
                                :style="{
                                  backgroundColor:
                                    currentOrgData.colors.primaryLight,
                                  color: currentOrgData.colors.primary,
                                }"
                                >Automation Addict</span
                              >
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3">
                          ...
                          <div class="flex-1 min-w-0">
                            ...
                            >
                              <div
                                class="h-3 bg-gray-300 rounded-full w-12"
                              ></div>
                              <div
                                class="h-3 bg-gray-300 rounded-full w-14"
                              ></div>
                              <div
                                class="h-3 bg-gray-300 rounded-full w-12"
                              ></div>
...
.tooltip-trigger::after {
  ...
  transition:
    opacity 0.2s ease,
    transform 0.2s ease; /* Removed left/bottom transition for instant movement */
  ...
  box-shadow:
    ...
    /* Purple glow */ 0 0 20px rgba(244, 114, 182, 0.4),
    /* Pink glow */ 0 0 30px rgba(251, 191, 36, 0.35),
    /* Yellow glow */ 0 0 40px rgba(52, 211, 153, 0.25); /* Green glow */
