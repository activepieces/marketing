...
const iconComponent = computed(() =>
  props.icon === "stack-simple" ? PhStackSimple : PhStack,
);
...
<template>
  <div
    class="bg-white border border-[rgba(49,43,96,0.1)] rounded-2xl p-6 sm:p-8 md:p-10 flex flex-col"
  >
    <!-- Icon -->
    <div class="flex justify-center mb-4">
      <component :is="iconComponent" :size="48" class="text-[#11A7B3]" />
    </div>
    ...
    <!-- Subtitle -->
    <p v-if="subtitle" class="text-xl text-primary-dark/80 text-center mt-1">
      {{ subtitle }}
    </p>
    ...
    <div class="text-center mb-6">
      <div class="font-sentient font-bold text-[32px] text-primary-dark">
        {{ price }}
      </div>
      <div v-if="priceSubtext" class="text-base text-primary-dark/60 mt-1">
        {{ priceSubtext }}
      </div>
    </div>
    ...
    <a
      ...
      :class="[
        ...
        ctaStyle === 'solid'
          ? 'bg-primary-dark text-white hover:bg-[#3d3575]'
          : 'border-2 border-[rgba(49,43,96,0.1)] text-primary-dark hover:bg-primary-dark/5',
      ]"
    >
      {{ ctaText }}
    </a>
      ...
      >
        ...
        <div class="flex-1 relative">
          <span
            ...
            class="font-semibold underline bg-gradient-to-r from-primary-dark to-[#11a7b3] bg-clip-text text-transparent cursor-help"
            data-feature-item
            @mouseenter="
              showTooltip(`flat-${idx}`, feature.description, $event)
            "
          ...
          <span v-else class="text-primary-dark/80">
            {{ feature.text }}
          </span>
          ...
          <Teleport to="body">
            <Transition name="tooltip-fade">
              <div
                ...
                data-tooltip
                class="fixed w-[calc(100vw-1rem)] sm:w-72 max-w-sm p-3 sm:p-4 bg-white text-primary-dark text-xs sm:text-sm rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.15)] border border-[rgba(49,43,96,0.15)] z-[9999] pointer-events-auto"
    ...
    >
      <div
        ...
        :class="{
          'overflow-hidden max-h-[420px]':
            collapsible && !isFeaturesExpanded && !isMobile,
        }"
      >
        <template
          v-for="category in collapsible
            ? getVisibleCategories(categories)
            : categories.map((c, i) => ({ ...c, originalIndex: i }))"
          :key="category.originalIndex"
        >
          <div>
            <h4
              class="uppercase text-sm font-medium text-primary-dark/40 mb-3 tracking-wide"
            >
              {{ category.category }}
            </h4>
              ...
              >
                ...
                <div class="flex-1 relative">
                  <span
                    ...
                    @mouseenter="
                      showTooltip(
                        `cat-${category.originalIndex}-${idx}`,
                        feature.description,
                        $event,
                      )
                    "
      ...
      >
        <div
          class="h-28 bg-gradient-to-b from-transparent to-white pointer-events-none absolute w-full bottom-0 left-0"
        ></div>
