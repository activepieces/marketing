<template>
  ...
  >
    <!-- Subtle grid pattern -->
    <div class="absolute inset-0 opacity-[0.03]" style="background-image: url('data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2220%22 height=%2220%22><rect fill=%22white%22 width=%221%22 height=%221%22/></svg>'); background-size: 20px 20px;"></div>
    
    <!-- Gradient glow -->
    <div class="absolute -top-20 -right-20 w-60 h-60 bg-violet-500/20 rounded-full blur-3xl"></div>
    <div class="absolute -bottom-20 -left-20 w-60 h-60 bg-blue-500/15 rounded-full blur-3xl"></div>

    <!-- ALL BADGES COLLECTED VIEW -->
    <Transition name="fade-scale">
      <div v-if="allCollected" class="absolute inset-0 z-50 overflow-hidden rounded-2xl">
        <!-- Dark base with golden accent -->
        <div class="absolute inset-0 bg-gradient-to-b from-[#0a0a0a] via-[#0f0f0f] to-[#0a0a0a]"></div>
        
        <!-- Golden light beam from top -->
        <div class="absolute top-0 left-1/2 -translate-x-1/2 w-40 h-full bg-gradient-to-b from-amber-400/20 via-amber-400/5 to-transparent blur-2xl"></div>
        
        <!-- Main content -->
        <div class="relative h-full flex flex-col items-center justify-center">
          <!-- Big score reveal -->
          <div class="text-center mb-6">
            <div class="text-8xl font-black text-transparent bg-clip-text bg-gradient-to-b from-amber-200 to-amber-500 score-reveal">
              10/10
            </div>
            <div class="text-amber-400/60 text-sm font-medium tracking-widest uppercase mt-2">Perfect</div>
          </div>
          
          <!-- Fun message -->
          <p class="text-white/60 text-lg mb-6 italic">That satisfying feeling? Your team wants it too 💜</p>
          ...
          <div class="relative h-20 w-full max-w-xs mb-8">
            <div 
              ...
              :key="badge.id"
              class="absolute text-3xl badge-arc"
              :style="{ 
                left: (10 + index * 9) + '%',
        ...
        <div class="absolute inset-0 pointer-events-none overflow-hidden">
          <div 
            v-for="i in 15" 
            :key="'p-'+i"
            class="absolute w-1 h-1 bg-amber-400 rounded-full particle-rise"
            :style="{
    ...
    <!-- START SCREEN -->
    <div v-if="!gameStarted" class="absolute inset-0 z-40 overflow-hidden rounded-2xl bg-white">
      <!-- Mesh gradient background -->
      <svg class="absolute inset-0 w-full h-full opacity-60" viewBox="0 0 400 420" preserveAspectRatio="xMidYMid slice">
        <defs>
          <radialGradient id="intro-mesh1" cx="20%" cy="20%">
            <stop offset="0%" stop-color="#ddd6fe" stop-opacity="0.8"/>
            <stop offset="100%" stop-color="#ddd6fe" stop-opacity="0"/>
          </radialGradient>
          <radialGradient id="intro-mesh2" cx="80%" cy="80%">
            <stop offset="0%" stop-color="#fde68a" stop-opacity="0.6"/>
            <stop offset="100%" stop-color="#fde68a" stop-opacity="0"/>
          </radialGradient>
          <radialGradient id="intro-mesh3" cx="70%" cy="20%">
            <stop offset="0%" stop-color="#fbcfe8" stop-opacity="0.5"/>
            <stop offset="100%" stop-color="#fbcfe8" stop-opacity="0"/>
          </radialGradient>
        </defs>
        <rect width="100%" height="100%" fill="url(#intro-mesh1)"/>
        <rect width="100%" height="100%" fill="url(#intro-mesh2)"/>
        <rect width="100%" height="100%" fill="url(#intro-mesh3)"/>
      </svg>
      ...
      <div class="relative h-full flex flex-col items-center justify-center px-8">
        ...
        <div class="relative mb-8">
          <!-- Glowing ring -->
          <div class="absolute inset-0 w-32 h-32 -m-4 rounded-full bg-gradient-to-br from-violet-200 to-amber-200 opacity-50 blur-2xl"></div>
          
          <!-- Main badge circle -->
          <div class="relative w-24 h-24 rounded-full bg-gradient-to-br from-violet-100 to-amber-50 border border-white/80 shadow-2xl shadow-violet-200/30 flex items-center justify-center">
            <span class="text-5xl">🏆</span>
          </div>
          
          <!-- Orbiting mini badges -->
          <div class="absolute -top-2 -right-2 w-9 h-9 rounded-full bg-white shadow-lg flex items-center justify-center text-lg border border-gray-100 intro-orbit-1">⭐</div>
          <div class="absolute -bottom-1 -left-3 w-8 h-8 rounded-full bg-white shadow-lg flex items-center justify-center text-base border border-gray-100 intro-orbit-2">🧠</div>
          <div class="absolute top-1/2 -right-5 w-8 h-8 rounded-full bg-white shadow-lg flex items-center justify-center text-base border border-gray-100 intro-orbit-3">💪</div>
        </div>
        
        <!-- Text -->
        <div class="text-center mb-8">
          <h1 class="text-2xl font-bold text-gray-900 mb-2">Badge Collection</h1>
          <p class="text-gray-400 text-sm">Unlock all 10 achievements</p>
        </div>
        
        <!-- Start button -->
        <button 
          @click="gameStarted = true"
          class="group relative px-10 py-4 bg-gradient-to-br from-violet-600 via-violet-500 to-fuchsia-500 text-white font-semibold rounded-2xl transition-all shadow-2xl shadow-violet-500/30 hover:shadow-violet-500/50 hover:scale-105 overflow-hidden intro-btn-float"
        >
          <!-- Glow ring behind -->
          <div class="absolute -inset-1 bg-gradient-to-r from-violet-600 to-fuchsia-500 rounded-2xl blur-lg opacity-60 group-hover:opacity-80 transition-opacity intro-btn-glow"></div>
          <span class="relative z-10">Start Collecting</span>
          <!-- Shine effect -->
          <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/25 to-transparent intro-btn-shine"></div>
        </button>
      </div>
    </div>

    <!-- MAIN GAME VIEW -->
    <div v-if="gameStarted && !allCollected" class="relative h-full flex flex-col p-5">
      <!-- Compact Question Area - Lighter background -->
      <div class="bg-white/[0.08] border border-white/10 rounded-xl p-4 mb-4 shadow-lg">
        <!-- Difficulty Tabs -->
        <div class="flex items-center gap-1 mb-3 pb-3 border-b border-white/5">
          <button 
            ...
            :class="difficulty === index ? mode.activeClass : 'text-gray-500 hover:text-gray-400 hover:bg-white/5'"
          >
            {{ mode.label }}
          </button>
        </div>
    ...
    <Transition name="dramatic">
      ...
      >
        ...
        <div class="absolute inset-0 bg-red-900/90 animate-pulse-fast"></div>
        
        <!-- Warning stripes -->
        <div class="absolute inset-0 opacity-10" style="background: repeating-linear-gradient(45deg, transparent, transparent 10px, black 10px, black 20px);"></div>
        
        <!-- Content -->
        <div class="relative text-center p-6 warning-shake">
          <div class="text-6xl mb-4">⚠️</div>
          <h3 class="text-white font-black text-2xl mb-2">Reset progress?</h3>
          <p class="text-red-200 text-sm mb-6">All badges will be lost forever</p>
          ...
          <div class="flex gap-3 justify-center">
            ...
            <button 
              ...
              class="px-5 py-2.5 bg-red-500 text-white text-sm font-bold rounded-lg hover:bg-red-400 transition-all animate-pulse"
            >
              Reset 💀
            </button>
          </div>
    ...
    <Transition name="dramatic">
      ...
      >
        ...
        <div class="flex items-center justify-between gap-4">
          <!-- Question -->
          <div class="font-bold text-white tracking-tight font-mono" :class="questionSize">
            <span v-html="currentQuestion"></span> = <span class="text-violet-400">?</span>
          </div>
            ...
            <button 
              ...
              :class="[
                selectedAnswer === option 
                  ? (option === correctAnswer ? 'bg-emerald-500/20 border-emerald-500 text-emerald-400' : 'bg-red-500/20 border-red-500 text-red-400 shake')
      ...
      <div class="flex-1 flex flex-col bg-gradient-to-b from-violet-500/5 to-violet-500/10 -mx-5 -mb-5 px-5 pb-5 pt-4 rounded-t-3xl border-t border-violet-500/20">
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center gap-2">
            <span class="text-xs text-violet-300/70 uppercase tracking-wider font-medium">🏆 Your Badges</span>
            <button 
              @click="tryReset"
              ...
              title="Reset"
            >
              <svg class="w-3 h-3 text-violet-300/50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div 
            ...
            @click="cheatUnlockAll"
          >
            <div class="text-xs text-violet-300/70 font-mono group-hover:text-violet-300 transition-colors">{{ unlockedCount }}/{{ badges.length }}</div>
            <!-- Secret cheat tooltip -->
            <div class="absolute -top-7 left-1/2 -translate-x-1/2 px-2 py-1 bg-white text-black text-[10px] font-bold rounded-full opacity-0 group-hover:opacity-100 transition-all scale-90 group-hover:scale-100 whitespace-nowrap pointer-events-none">
              Cheat? 🤫
            </div>
          </div>
        </div>
        ...
        <div class="flex-1 grid grid-cols-5 grid-rows-2 gap-3">
          ...
          >
            ...
            >
              <span class="text-3xl" :class="badge.unlocked ? '' : 'grayscale opacity-50'">
                {{ badge.unlocked ? badge.emoji : '?' }}
              </span>
              <span class="text-[9px] text-gray-400 font-medium truncate max-w-full px-1">
            ...
            <Transition name="tooltip">
              <div 
                ...
                class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-white text-gray-900 text-xs rounded-lg whitespace-nowrap z-50 shadow-xl"
              >
                <div class="font-semibold">{{ badge.unlocked ? badge.name : '???' }}</div>
                <div class="text-gray-500 text-[10px]">{{ badge.unlocked ? badge.description : badge.hint }}</div>
    ...
    <Transition name="celebration">
      ...
      >
        ...
        <div class="absolute inset-0 flex items-center justify-center">
          <div class="w-40 h-40 bg-violet-500/30 rounded-full blur-3xl animate-pulse"></div>
        </div>
        ...
        <div class="relative text-center celebration-pop">
          ...
          <div class="relative inline-block mb-4">
            <div class="text-7xl animate-bounce-badge">{{ celebrationBadge?.emoji }}</div>
            <div class="absolute inset-0 text-7xl blur-lg opacity-50">{{ celebrationBadge?.emoji }}</div>
          </div>
          
          <!-- Text -->
          <div class="space-y-1">
            <div class="text-violet-300 text-xs font-medium uppercase tracking-widest">🏆 Badge Unlocked!</div>
            <div class="text-white font-bold text-2xl">{{ celebrationBadge?.name }}</div>
            <div class="text-gray-400 text-sm">{{ celebrationBadge?.description }}</div>
          </div>
          
          <!-- Sparkle ring -->
          <div class="absolute -inset-8 border-2 border-violet-500/30 rounded-full animate-ping-slow"></div>
        </div>
      </div>
    </Transition>
  </div>
...
const currentQuestionData = computed(() => {
  const pool = questionsByDifficulty[difficulty.value] || questionsByDifficulty[0]
...
const badges = ref([
  { id: 'welcome', emoji: '👋', name: 'Hello Friend!', description: 'Welcome to the party!', hint: 'Already yours!', unlocked: true, justUnlocked: false },
  { id: 'clicked', emoji: '👆', name: 'First Click', description: 'You clicked something!', hint: 'Click any answer', unlocked: false, justUnlocked: false },
  { id: 'genius', emoji: '🧠', name: 'Math Genius', description: '1+1=2. Nobel Prize!', hint: 'Get one right', unlocked: false, justUnlocked: false },
  { id: 'pizza', emoji: '🍕', name: 'Pizza Lover', description: 'Pizza > Math', hint: 'Pick the tasty one', unlocked: false, justUnlocked: false },
  { id: 'rebel', emoji: '😈', name: 'Rebel', description: 'Rules are boring', hint: 'Wrong answer', unlocked: false, justUnlocked: false },
  { id: 'curious', emoji: '🔍', name: 'Curious', description: 'Testing options', hint: 'Try 2 options', unlocked: false, justUnlocked: false },
  { id: 'explorer', emoji: '🗺️', name: 'Explorer', description: 'Tried everything', hint: 'Try all 4', unlocked: false, justUnlocked: false },
  { id: 'persistent', emoji: '💪', name: 'Persistent', description: 'Never stops!', hint: '3 attempts', unlocked: false, justUnlocked: false },
  { id: 'dedicated', emoji: '⭐', name: 'Dedicated', description: 'Badge hunter!', hint: '5 clicks', unlocked: false, justUnlocked: false },
  { id: 'champion', emoji: '🏆', name: 'Champion', description: 'Got them all!', hint: 'Collect all', unlocked: false, justUnlocked: false },
])
  ...
  celebrationTimeout = setTimeout(() => {
    ...
    setTimeout(() => {
      if (pendingBadges.value.length > 0) {
        ...
        const nonChampionBadges = badges.value.filter(b => b.id !== 'champion')
...
@keyframes confetti-fall {
  ...
  100% {
    transform: translateY(450px) rotate(720deg) translateX(var(--drift)) scale(0.5);
    opacity: 0;
  }
}
...
@keyframes float-around {
  ...
  75% { transform: translate(5px, -25px) rotate(3deg); }
}
...
.dramatic-enter-active, .dramatic-leave-active {
  transition: all 0.2s ease-out;
}
.dramatic-enter-from, .dramatic-leave-to {
  opacity: 0;
}
