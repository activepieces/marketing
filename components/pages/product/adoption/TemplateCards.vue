    ...
    >
      <div
        ...
        :style="{ 
          ...
          backgroundColor: hoveredCircle === index ? '#0a0a0a' : colors[circle.colorIndex],
          boxShadow: hoveredCircle === index ? '0 12px 40px rgba(0,0,0,0.4)' : 'none',
          transform: hoveredCircle === index ? 'translate(-50%, -50%)' : 'translate(0, 0)',
      ...
      >
        ...
        >
          ...
          <p class="text-white font-semibold text-base leading-tight text-center">
...
const departments = [
  { short: 'Sales', useCases: ['Qualify inbound leads', 'Research before calls', 'Score hot prospects'] },
  { short: 'HR', useCases: ['Screen CVs instantly', 'Generate interview packs', 'Summarize employee feedback'] },
  { short: 'Legal', useCases: ['Flag risky clauses', 'Monitor contract expiry', 'Automate due diligence'] },
  { short: 'Marketing', useCases: ['Repurpose viral content', 'Draft personalized emails', 'Analyze Reddit trends'] },
  { short: 'Finance', useCases: ['Chase overdue invoices', 'Categorize receipt expenses', 'Generate monthly reports'] },
  { short: 'Support', useCases: ['Route tickets smartly', 'Transcribe support calls', 'Send sentiment alerts'] },
  { short: 'Eng', useCases: ['Summarize GitHub activity', 'Create issues from forms', 'Post standup updates'] },
  { short: 'IT', useCases: ['Provision new accounts', 'Index docs for RAG', 'Monitor system health'] },
  { short: 'Ops', useCases: ['Track inventory levels', 'Alert low stock', 'Validate compliance filings'] },
]
...
const parseAsciiArt = () => {
  ...
  const isAdjacentToLogo = (row, col) => {
    for (let dr = -1; dr <= 1; dr++) {
      for (let dc = -1; dc <= 1; dc++) {
        ...
        const nr = row + dr
        const nc = col + dc
        if (nr >= 0 && nr < ASCII_HEIGHT && nc >= 0 && nc < ASCII_ART[nr].length) {
  ...
  const candidates = topRightIsolated.length > 0 ? topRightIsolated : isolatedBgList
  ...
  const specialIdx = bgList.findIndex(b => b.x === special.x && b.y === special.y)
  if (specialIdx !== -1) bgList.splice(specialIdx, 1)
  
  return { circles: circlesList, background: bgList, special }
}

const { circles, background: backgroundShapes, special: specialShape } = parseAsciiArt()
</script>
