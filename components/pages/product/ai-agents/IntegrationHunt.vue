<template>
  <div>
    <div 
      ...
      :style="{ background: 'linear-gradient(180deg, #faf5ff 0%, #f3e8ff 50%, #ede9fe 100%)' }"
      tabindex="0"
    ...
    >
      <div class="relative">
        <img 
          ...
          :class="[
            ...
            target.isTiny ? 'w-4 h-4' : target.isGiant ? 'w-10 h-10' : 'w-6 h-6'
          ]"
          :alt="target.displayName"
        />
        <span v-if="target.isMalware" class="text-lg">{{ target.caught ? '💀' : '🐛' }}</span>
      </div>
      <span 
        class="font-medium whitespace-nowrap"
        :class="[
          getTargetTextClass(target),
          target.isTiny ? 'text-xs' : target.isGiant ? 'text-base' : 'text-sm'
        ]"
      >
      ...
      <span 
        ...
        :class="[
          target.isGiant 
            ? (target.health > 2 ? 'bg-orange-500 text-white' : 'bg-red-500 text-white animate-pulse')
            : (target.health === target.maxHealth ? 'bg-blue-500 text-white' : 'bg-orange-500 text-white animate-pulse')
        ]"
      >
        {{ target.health }}×
      </span>
    ...
    <TransitionGroup name="founder">
      <div
        ...
        :style="{ left: founder.x + 'px', top: founder.y + 'px', transform: 'translate(-50%, -50%)' }"
      >
        <!-- Photo + angry emoji -->
        <div class="relative">
          <div class="w-12 h-12 rounded-full overflow-hidden shadow-lg bg-gray-100">
            <img 
              :src="founder.avatar" 
          ...
          <span class="absolute -top-2 -right-2 text-2xl drop-shadow-md">😡</span>
        </div>
        
        <!-- Name below -->
        <span class="mt-1 text-xs font-bold text-gray-800 bg-white/90 px-2 py-0.5 rounded-full shadow">{{ founder.name?.split(' ')[0] }}</span>
      </div>
    </TransitionGroup>
    ...
    <TransitionGroup name="devil">
      <div
        ...
        :style="{ left: devil.x + 'px', top: devil.y + 'px', transform: 'translate(-50%, -50%)' }"
      >
    ...
    <div 
      ...
      :style="{ left: playerX + 'px', transform: 'translateX(-50%)', transition: 'left 0.05s linear' }"
    >
      <div class="relative">
        <div class="w-12 h-8 relative">
          ...
          ></div>
          <div class="absolute top-1 left-1/2 -translate-x-1/2 w-4 h-3 rounded-t-full bg-purple-300/80"></div>
        </div>
        <div 
    ...
    <div class="absolute top-4 right-4 text-right">
      <div class="text-3xl font-bold text-purple-600 tabular-nums">{{ score }}</div>
      <div class="text-[10px] text-purple-400 font-medium uppercase tracking-wider">caught</div>
    </div>
    ...
    <div class="absolute top-4 left-4">
      <div class="flex gap-1">
        <div 
          ...
          :class="i <= lives ? 'opacity-100 scale-100' : 'opacity-20 scale-90 grayscale'"
        >
          <svg viewBox="0 0 24 24" fill="currentColor" class="text-red-500 drop-shadow-sm">
            <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
          </svg>
          <!-- Crack overlay for lost hearts -->
          <svg v-if="i > lives" viewBox="0 0 24 24" class="absolute inset-0 w-full h-full text-gray-600">
            <path d="M12 4 L14 10 L10 12 L13 18" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round"/>
          </svg>
        </div>
      </div>
    ...
    <div class="absolute top-14 left-4 group cursor-help">
      <div class="flex items-center gap-1.5 text-purple-400">
        <span class="text-lg font-bold tabular-nums">{{ remainingPieces }}</span>
        <span class="text-xs uppercase tracking-wider">left</span>
      </div>
      <div class="absolute left-0 top-full mt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-30">
    ...
    >
      ...
      <div class="absolute inset-0 pointer-events-none overflow-hidden">
        <div class="absolute top-8 left-8 text-2xl opacity-20 animate-float-random">📱</div>
        <div class="absolute top-12 right-12 text-xl opacity-15 animate-float-random" style="animation-delay: 0.5s">💬</div>
        <div class="absolute bottom-16 left-16 text-2xl opacity-20 animate-float-random" style="animation-delay: 1s">📊</div>
        <div class="absolute bottom-10 right-10 text-xl opacity-15 animate-float-random" style="animation-delay: 1.5s">📧</div>
        <div class="absolute top-1/3 left-6 text-lg opacity-10 animate-float-random" style="animation-delay: 0.8s">🗂️</div>
        <div class="absolute top-1/4 right-8 text-lg opacity-10 animate-float-random" style="animation-delay: 1.2s">📅</div>
      </div>
      
      <div class="text-center relative z-10">
        <div class="text-5xl mb-3 animate-bounce-slow">👾</div>
        <h2 class="text-slate-800 font-black text-2xl mb-5">Integration Invasion</h2>
        <button 
      ...
      >
        <div class="text-center max-w-sm px-6">
          ...
          <div class="mb-8 flex justify-center">
            <div class="inline-flex items-center gap-3 px-5 py-4 rounded-2xl shadow-2xl bg-white/95">
              <!-- Placeholder icon -->
              <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0">
                <span v-if="challengeType === 'bugs'" class="text-2xl">🐛</span>
                <div v-else class="w-6 h-6 rounded bg-gray-300"></div>
              </div>
              
              <!-- Placeholder text lines -->
              <div v-if="challengeType !== 'bugs'" class="flex flex-col gap-1.5">
                <div class="w-14 h-2.5 bg-gray-300 rounded"></div>
                <div class="w-8 h-2 bg-gray-200 rounded"></div>
              </div>
              <span v-else class="text-base font-bold text-red-500">AVOID!</span>
              
              <!-- ANIMATED BADGE - the key difference -->
              <div v-if="challengeType === 'armored'" class="animate-badge-drop ml-2">
                <span class="text-2xl font-black px-4 py-2 rounded-xl bg-blue-500 text-white shadow-xl animate-pulse-glow">2×</span>
              </div>
              <div v-if="challengeType === 'speed'" class="animate-badge-zoom ml-2">
                <span class="text-5xl drop-shadow-xl animate-flash">⚡</span>
              </div>
              <div v-if="challengeType === 'bugs'" class="animate-badge-shake ml-2">
                <span class="text-4xl animate-wiggle-continuous">💀</span>
              </div>
              <div v-if="challengeType === 'ghost'" class="animate-badge-fade ml-2">
                <span class="text-5xl opacity-40 animate-pulse">👻</span>
              </div>
              <div v-if="challengeType === 'tiny'" class="animate-badge-zoom ml-2">
                <span class="text-2xl drop-shadow-xl">🔬</span>
              </div>
              <div v-if="challengeType === 'zigzag'" class="animate-badge-shake ml-2">
                <span class="text-5xl animate-zigzag">〰️</span>
              </div>
              <div v-if="challengeType === 'giant'" class="animate-badge-drop ml-2">
                <span class="text-2xl font-black px-4 py-2 rounded-xl bg-orange-500 text-white shadow-xl animate-pulse-glow">5×</span>
                <span class="text-4xl ml-2">👑</span>
              </div>
              <div v-if="challengeType === 'swarm'" class="animate-badge-zoom ml-2 flex gap-1">
                <span class="text-3xl animate-bounce" style="animation-delay: 0s">📦</span>
          ...
          <!-- Game title -->
          <div class="mb-6">
            <h2 class="text-white font-black text-4xl uppercase tracking-tight drop-shadow-lg">{{ challengeTitle }}</h2>
          ...
          <p class="text-white/40 text-xs mt-5 uppercase tracking-widest">[ SPACE ]</p>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="gameover">
      ...
      >
        ...
        <div class="absolute inset-0 pointer-events-none overflow-hidden">
          <div v-for="i in 12" :key="i" class="absolute text-3xl opacity-20 animate-float-random" :style="{ left: (i * 8) + '%', top: (10 + (i % 5) * 18) + '%', animationDelay: (i * 0.2) + 's' }">
            {{ getGameOverEmoji(i) }}
          </div>
        </div>
        ...
        <div class="text-center relative z-10">
          ...
          <p class="text-white/80 text-lg font-bold uppercase tracking-widest mb-4">
            {{ getGameOverTitle }}
          </p>
          
          <!-- Score with flair -->
          <div class="mb-4">
            <p class="text-white font-black text-7xl tabular-nums drop-shadow-lg">{{ score }}</p>
            <p class="text-white/60 text-sm uppercase tracking-widest mt-1">integrations hunted</p>
          </div>
          
          <!-- Unlocked challenges showcase -->
          <div v-if="unlockedChallengesCount > 0" class="mb-6 flex justify-center gap-2 flex-wrap">
            <span v-if="armoredActive" class="text-2xl" title="Armored">🛡️</span>
            ...
            <span v-if="zigzagActive" class="text-2xl" title="Zigzag">〰️</span>
    ...
    <div class="absolute bottom-20 left-1/2 -translate-x-1/2 text-center pointer-events-none">
      <div class="text-[70px] font-bold text-purple-200/30 leading-none tabular-nums">
        {{ totalCount }}+
      </div>
    </div>
  </div>
...
const getGameOverBg = computed(() => {
  if (score.value >= 125) return 'background: radial-gradient(circle at center, rgba(168, 85, 247, 0.95), rgba(88, 28, 135, 0.98))'
  if (score.value >= 80) return 'background: radial-gradient(circle at center, rgba(234, 88, 12, 0.95), rgba(124, 45, 18, 0.98))'
  if (score.value >= 45) return 'background: radial-gradient(circle at center, rgba(59, 130, 246, 0.95), rgba(30, 58, 138, 0.98))'
  if (score.value >= 20) return 'background: radial-gradient(circle at center, rgba(34, 197, 94, 0.9), rgba(22, 101, 52, 0.98))'
...
const getGameOverButtonClass = computed(() => {
  if (score.value >= 80) return 'bg-purple-600 hover:bg-purple-500 text-white border-purple-800'
  if (score.value >= 45) return 'bg-blue-600 hover:bg-blue-500 text-white border-blue-800'
  if (score.value >= 20) return 'bg-green-600 hover:bg-green-500 text-white border-green-800'
...
const showChallenge = (type) => {
  ...
  if (type === 'armored') {
    ...
    challengeBg.value = 'linear-gradient(135deg, rgba(37, 99, 235, 0.95), rgba(59, 130, 246, 0.95))'
    challengePreviewClass.value = 'bg-blue-50 border-2 border-blue-300'
    challengePreviewTextClass.value = 'text-gray-700'
    challengeButtonClass.value = 'bg-blue-600 hover:bg-blue-500 text-white shadow-xl border-blue-800'
    ...
    challengeBg.value = 'linear-gradient(135deg, rgba(245, 158, 11, 0.95), rgba(251, 191, 36, 0.95))'
    challengePreviewClass.value = 'bg-amber-50 border-2 border-amber-300'
    challengePreviewTextClass.value = 'text-amber-700'
    challengeButtonClass.value = 'bg-amber-500 hover:bg-amber-400 text-white shadow-xl border-amber-700'
    ...
    challengeBg.value = 'linear-gradient(135deg, rgba(220, 38, 38, 0.95), rgba(239, 68, 68, 0.95))'
    challengePreviewClass.value = 'bg-red-50 border-2 border-red-300'
    challengePreviewTextClass.value = 'text-red-600 font-bold'
    challengeButtonClass.value = 'bg-red-600 hover:bg-red-500 text-white shadow-xl border-red-800'
    ...
    challengeBg.value = 'linear-gradient(135deg, rgba(107, 114, 128, 0.95), rgba(156, 163, 175, 0.95))'
    challengePreviewClass.value = 'bg-gray-50 border-2 border-gray-300 opacity-40'
    challengePreviewTextClass.value = 'text-gray-500'
    challengeButtonClass.value = 'bg-gray-600 hover:bg-gray-500 text-white shadow-xl border-gray-800'
    ...
    challengeBg.value = 'linear-gradient(135deg, rgba(168, 85, 247, 0.95), rgba(192, 132, 252, 0.95))'
    challengePreviewClass.value = 'bg-purple-50 border-2 border-purple-300 scale-75'
    challengePreviewTextClass.value = 'text-purple-600'
    challengeButtonClass.value = 'bg-purple-600 hover:bg-purple-500 text-white shadow-xl border-purple-800'
    ...
    challengeBg.value = 'linear-gradient(135deg, rgba(6, 182, 212, 0.95), rgba(34, 211, 238, 0.95))'
    challengePreviewClass.value = 'bg-cyan-50 border-2 border-cyan-300'
    challengePreviewTextClass.value = 'text-cyan-600'
    challengeButtonClass.value = 'bg-cyan-600 hover:bg-cyan-500 text-white shadow-xl border-cyan-800'
    ...
    challengeBg.value = 'linear-gradient(135deg, rgba(234, 88, 12, 0.95), rgba(249, 115, 22, 0.95))'
    challengePreviewClass.value = 'bg-orange-50 border-2 border-orange-400 scale-125'
    challengePreviewTextClass.value = 'text-orange-600 font-black'
    challengeButtonClass.value = 'bg-orange-600 hover:bg-orange-500 text-white shadow-xl border-orange-800'
    ...
    challengeBg.value = 'linear-gradient(135deg, rgba(236, 72, 153, 0.95), rgba(244, 114, 182, 0.95))'
    challengePreviewClass.value = 'bg-pink-50 border-2 border-pink-300'
    challengePreviewTextClass.value = 'text-pink-600'
    challengeButtonClass.value = 'bg-pink-600 hover:bg-pink-500 text-white shadow-xl border-pink-800'
...
onMounted(async () => {
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      console.log(`Loaded ${piecesWithFounders.value.length} pieces with founder photos (prioritized)`)
    }
  } catch (error) {
...
const update = (currentTime) => {
  ...
  if (keysPressed.value.left) {
    targetPlayerX.value = Math.max(30, targetPlayerX.value - playerSpeed * deltaTime)
  }
  if (keysPressed.value.right) {
    targetPlayerX.value = Math.min(490, targetPlayerX.value + playerSpeed * deltaTime)
  }
    ...
    for (const target of activeTargets.value) {
      ...
      if (overlapX && overlapY) {
        ...
        if (target.health <= 0) {
          ...
          setTimeout(() => {
            const idx = activeTargets.value.findIndex(t => t.id === target.id)
            if (idx !== -1) {
...
const getTargetTextClass = (target) => {
  if (target.caught) {
    return target.isMalware ? 'text-red-400 line-through' : 'text-green-600 line-through'
  }
...
const handleMouseMove = (e) => {
  if (!gameContainer.value || !gameStarted.value || gameOver.value || gamePaused.value) return
...
.animate-badge-shake {
  animation: badgeShake 0.6s ease-out forwards, badgeWiggle 0.3s ease-in-out 0.6s infinite;
}
